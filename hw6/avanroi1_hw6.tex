...
\begin{enumerate}
	...
	\begin{gather}
		...
		\text{Now we want to show: }M_{X+Y+X-Y}(s)=M_{X+Y}(s)M_{X-Y}(s) \\
		M_{X+Y+X-Y}(s) = M_{2X}(s)\\
		M_{X+Y}(s)M_{X-Y}(s) = M_X(s)M_Y(s)M_X(s)M_{-Y}(s)\\
		M_Y(s)M_{-Y}(s) \text{ with Y and -Y following Unif(0,1) and Unif(-1,0)}\\
		\frac{e^{sb} - e^{sa}}{s(b-a)} => \frac{e^{s} - 1}
		{s(1)} , \frac{1 - e^{-s}}{s(1)}
